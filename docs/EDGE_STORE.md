...
6. Готовые материалы карточки находятся в [`docs/edge-store/`](edge-store/): маленькая плитка 440×280, большая плитка 1400×560 и два скриншота 1280×800 в тёмной теме.
...
**Краткое описание:** Умная плитка с нужными сайтами — быстрый поиск, адаптивная тема и спокойный интерфейс.

**Описание:** Radial New Tab заменяет стандартную стартовую страницу Edge на лёгкое персональное пространство. Умная плитка учитывает частоту, недавность, день недели и время суток, объясняет верхние подсказки и позволяет закрепить важные ссылки. Интерфейс автоматически следует системной светлой или тёмной теме и выбирает русский или английский язык браузера. Доступны коллекции ссылок, импорт выбранной папки закладок с распределением по вложенным папкам, Яндекс, Bing, Google и DuckDuckGo, импорт и экспорт, режим фокуса и ручная настройка внешнего вида.
...
**English description:** Radial New Tab is a calm, adaptive Microsoft Edge new tab with a smart shortcut grid, clock, and search. The grid learns from shortcut frequency, recency, weekday, and time of day, explains its top suggestions, and lets users pin important links. Add, edit, reorder, import, and export shortcuts; organize them into optional collections; import a selected Edge bookmarks folder; and optionally add sites from Edge’s top sites list after permission. Links and settings stay in Edge storage; Radial uses no analytics or own servers.
...
3. В Settings проверить светлую/тёмную тему, язык, умную плитку, фоновое изображение, экспорт/импорт и Focus mode.
